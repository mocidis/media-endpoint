...
void streamer_config_file_source(endpoint_t *streamer, char *file_name) {
    pjmedia_port *port;
    CHECK(__FILE__, pjmedia_wav_player_port_create(streamer->pool, file_name, 0, 0, -1, &streamer->filein.port));

    if(streamer->stream != NULL) {
        CHECK(__FILE__, pjmedia_stream_get_port(streamer->stream, &port));
        CHECK(__FILE__, pjmedia_master_port_create(streamer->pool,
                                streamer->filein.port, 
                                port,
                                0,
                                &streamer->filein.mport));
    }
...
void streamer_config_dev_source(endpoint_t *streamer, int idx) {
    //pjmedia_port *port;
    //CHECK(__FILE__, pjmedia_stream_get_port(streamer->stream, &port));
    PJ_LOG(3, (__FILE__, "kkkkk %d - %d - %d", streamer->ci->clock_rate, streamer->ci->channel_cnt, 
...
void streamer_start(endpoint_t *streamer) {
    ...
    if( streamer->state == EPS_STOP ) {
        ...
        case EPT_FILE:
            ...
            //CHECK(__FILE__, pjmedia_stream_start(streamer->stream));
            CHECK(__FILE__, pjmedia_master_port_start(streamer->filein.mport));
            streamer->state = EPS_START;
            break;
        case EPT_DEV:
            PJ_LOG(3, (__FILE__, "Start stream from sound dev"));
            CHECK(__FILE__, pjmedia_stream_get_port(streamer->stream, &port));
            CHECK(__FILE__, pjmedia_snd_port_connect(streamer->devin.snd_port, port));
            //CHECK(__FILE__, pjmedia_stream_start(streamer->stream));
            streamer->state = EPS_START;
            break;
        default:
...
void streamer_stop(endpoint_t *streamer) {
    ...
    if( streamer->state == EPS_START) {
        switch(streamer->type) {
        case EPT_FILE:
            CHECK(__FILE__, pjmedia_master_port_stop(streamer->filein.mport));
            streamer->state = EPS_STOP;
            break;
        case EPT_DEV:
            CHECK(__FILE__, pjmedia_snd_port_disconnect(streamer->devin.snd_port));
            streamer->state = EPS_STOP;
            break;
        default:
            PJ_LOG(3, (__FILE__, "Unknown endpoint type"));
...
void receiver_start(endpoint_t *receiver) {
    ...
    if( receiver->state == EPS_STOP ) {
        ...
        case EPT_FILE:
            ...
            CHECK(__FILE__, pjmedia_master_port_start(receiver->fileout.mport));
            receiver->state = EPS_START;
            break;
        case EPT_DEV:
            PJ_LOG(3, (__FILE__, "Start stream to sound dev"));
            CHECK(__FILE__, pjmedia_stream_get_port(receiver->stream, &port));
            CHECK(__FILE__, pjmedia_snd_port_connect(receiver->devout.snd_port, port));
            receiver->state = EPS_START;
            break;
        default:
            PJ_LOG(3, (__FILE__, "Unknown endpoint type"));
...
void receiver_stop(endpoint_t *receiver) {
    ...
    if( receiver->state == EPS_START) {
        switch( receiver->type ) {
        case EPT_FILE:
            CHECK(__FILE__, pjmedia_master_port_stop(receiver->fileout.mport));
            receiver->state = EPS_STOP;
            break;
        case EPT_DEV: 
            CHECK(__FILE__, pjmedia_snd_port_disconnect(receiver->devout.snd_port));
            receiver->state = EPS_STOP;
            break;
        default:
            PJ_LOG(3, (__FILE__, "Unknown endpoint type"));
